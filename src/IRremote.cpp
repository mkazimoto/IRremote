...
void processIrSensor(irparams_t *irparamsLocal) {
    TIMER_RESET; // reset timer interrupt flag if required (currently only for Teensy and ATmega4809)

    // Read if IR Receiver -> SPACE [xmt LED off] or a MARK [xmt LED on]
    // digitalRead() is very slow. Optimisation is possible, but makes the code unportable
    uint8_t irdata = (uint8_t) digitalRead(irparamsLocal->recvpin);

    irparamsLocal->timer++;  // One more 50uS tick
    if (irparamsLocal->rawlen >= RAW_BUFFER_LENGTH) {
        irparamsLocal->rcvstate = IR_REC_STATE_OVERFLOW;  // Buffer overflow
    }
     ...
     */
//    switch (irparamsLocal->rcvstate) {
    //......................................................................
    if (irparamsLocal->rcvstate == IR_REC_STATE_IDLE) { // In the middle of a gap
        if (irdata == MARK) {
            if (irparamsLocal->timer < GAP_TICKS) {  // Not big enough to be a gap.
                irparamsLocal->timer = 0;
            } else {
                // Gap just ended; Record duration; Start recording transmission
                irparamsLocal->overflow = false;
                irparamsLocal->rawlen = 0;
                irparamsLocal->rawbuf[irparamsLocal->rawlen++] = irparamsLocal->timer;
                irparamsLocal->timer = 0;
                irparamsLocal->rcvstate = IR_REC_STATE_MARK;
            }
        }
    } else if (irparamsLocal->rcvstate == IR_REC_STATE_MARK) {  // Timing Mark
        if (irdata == SPACE) {   // Mark ended; Record time
            irparamsLocal->rawbuf[irparamsLocal->rawlen++] = irparamsLocal->timer;
            irparamsLocal->timer = 0;
            irparamsLocal->rcvstate = IR_REC_STATE_SPACE;
        }
    } else if (irparamsLocal->rcvstate == IR_REC_STATE_SPACE) {  // Timing Space
        if (irdata == MARK) {  // Space just ended; Record time
            irparamsLocal->rawbuf[irparamsLocal->rawlen++] = irparamsLocal->timer;
            irparamsLocal->timer = 0;
            irparamsLocal->rcvstate = IR_REC_STATE_MARK;

        } else if (irparamsLocal->timer > GAP_TICKS) {  // Space
            // A long Space, indicates gap between codes
            // Flag the current code as ready for processing
            // Switch to STOP
            // Don't reset timer; keep counting Space width
            irparamsLocal->rcvstate = IR_REC_STATE_STOP;
        }
    } else if (irparamsLocal->rcvstate == IR_REC_STATE_STOP) {  // Waiting; Measuring Gap
        if (irdata == MARK) {
            irparamsLocal->timer = 0;  // Reset gap timer
        }
    } else if (irparamsLocal->rcvstate == IR_REC_STATE_OVERFLOW) {  // Flag up a read overflow; Stop the State Machine
        irparamsLocal->overflow = true;
        irparamsLocal->rcvstate = IR_REC_STATE_STOP;
    }

#ifdef BLINKLED
    // If requested, flash LED while receiving IR data
    if (irparamsLocal->blinkflag) {
        if (irdata == MARK) {
            if (irparamsLocal->blinkpin) {
                digitalWrite(irparamsLocal->blinkpin, HIGH); // Turn user defined pin LED on
            } else {
                BLINKLED_ON();   // if no user defined LED pin, turn default LED pin for the hardware on
            }
        } else {
            if (irparamsLocal->blinkpin) {
                digitalWrite(irparamsLocal->blinkpin, LOW); // Turn user defined pin LED on
            } else {
